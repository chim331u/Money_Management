...
public class SessionStorageAccessor : IAsyncDisposable
{
        ...
        if (_accessorJsRef.IsValueCreated is false)
            _accessorJsRef =
                new Lazy<IJSObjectReference>(
                    await _jsRuntime.InvokeAsync<IJSObjectReference>("import", "/js/SessionStorageAccessor.js"));
    }
    ...
    public async Task<T> GetAllValueAsync<T>()
    {
        await WaitForReference();
        var result = await _accessorJsRef.Value.InvokeAsync<T>("getAll");
