...
public class IdentityAccessService : IIdentityAccessService
{
    ...
    public IdentityAccessService(ILogger<IdentityAccessService> logger, ApplicationContext context,
        IUtilityService utilityService, IAncillaryService anchillaryService)
    {
        _context = context;
    ...
    public async Task<ApiResponse<ICollection<ISA_Accounts>>> GetActiveIdentityAccountList()
    {
        try
        {
            ...
            return new ApiResponse<ICollection<ISA_Accounts>>(result,
                $"Successfully retrieved {result.Count} active identity accounts.");
        }
        catch (Exception ex)
        {
            _logger.LogError($"Error retrieving active identity accounts: {ex.Message}");
            return new ApiResponse<ICollection<ISA_Accounts>>(null,
                $"Error retrieving active identity accounts: {ex.Message}");
        }
    }
    ...
    public async Task<ApiResponse<ISA_Accounts>> GetIdentityAccount(int isaAccountId)
    {
        try
        {
            var result = await _context.ISA_Accounts.Include(c => c.ISA_User).Where(x => x.Id == isaAccountId)
                .FirstOrDefaultAsync();
            return new ApiResponse<ISA_Accounts>(result,
                $"Successfully retrieved identity account with ID {isaAccountId}.");
        }
        catch (Exception ex)
        {
            _logger.LogError($"Error retrieving identity account with ID {isaAccountId}: {ex.Message}");
            return new ApiResponse<ISA_Accounts>(null,
                $"Error retrieving identity account with ID {isaAccountId}: {ex.Message}");
        }
    }
    ...
    public async Task<ApiResponse<ISA_Accounts>> UpdateIdentityAccount(ISA_Accounts item)
    {
        try
        {
            ...
            return new ApiResponse<ISA_Accounts>(existingIdentityAccount,
                $"Successfully updated identity account with ID {item.Id}.");
        }
        catch (Exception ex)
        {
            _logger.LogError($"Error updating identity account with ID {item.Id}: {ex.Message}");
            return new ApiResponse<ISA_Accounts>(null,
                $"Error updating identity account with ID {item.Id}: {ex.Message}");
        }
    }
    ...
    public async Task<ApiResponse<ISA_Accounts>> AddIdentityAccount(ISA_Accounts item)
    {
        try
        {
            ...
            _context.ISA_Accounts.Update(item);
            await _context.SaveChangesAsync();
            return new ApiResponse<ISA_Accounts>(await _context.ISA_Accounts.FindAsync(item.Id),
                $"Successfully added identity account for user {item.ISA_User.Name}.");
        }
        catch (Exception ex)
        {
    ...
    public async Task<ApiResponse<bool>> DeleteIdentityAccount(int id)
    {
        try
        {
            var existingIdentityAccount = await _context.ISA_Accounts.Include(x => x.ISA_User).Where(x => x.Id == id)
                .FirstOrDefaultAsync();
            if (existingIdentityAccount == null)
            {
    ...
    public async Task<ApiResponse<string>> GetCleanPsw(int isa_account_id)
    {
        try
        {
            ...
            if (account != null)
                return new ApiResponse<string>(_utilityService.DecryptString(account),
                    $"Successfully retrieved password for account ID {isa_account_id}.");
        ...
        catch (Exception ex)
        {
            ...
            return new ApiResponse<string>(string.Empty,
                $"Error retrieving password for account ID {isa_account_id}: {ex.Message}");
        }
    }
    ...
    {
        var oldPassword = await _context.ISA_Accounts.Where(x => x.Id == item.Id).Select(x => x.Password)
            .FirstOrDefaultAsync();
        ...
        catch (Exception ex)
        {
            ...
            return new ApiResponse<string>(string.Empty,
                $"Error updating password for account ID {item.Id}: {ex.Message}");
        }

        var account = await _context.ISA_Accounts.Include(x => x.ISA_User).Where(x => x.Id == item.Id)
            .FirstOrDefaultAsync();
        ...
        try
        {
            ...
            return new ApiResponse<string>("Password Changed",
                $"Successfully changed password for account ID {item.Id}.");
        }
        catch (Exception ex)
        {
            _logger.LogError($"Error saving old password for account ID {item.Id}: {ex.Message}");
            return new ApiResponse<string>(string.Empty,
                $"Error saving old password for account ID {item.Id}: {ex.Message}");
        }
    }
    ...
    public async Task<ApiResponse<ICollection<ISA_PasswordsOld>>> ListAllOldPasswords(int accountId)
    {
        try
        {
            var result = await _context.ISA_PasswordsOld.Include(c => c.ISA_Account)
                .Where(x => x.IsActive && x.ISA_Account.Id == accountId).OrderByDescending(x => x.LastUpdatedDate)
                .ToListAsync();
            return new ApiResponse<ICollection<ISA_PasswordsOld>>(result,
                $"Successfully retrieved {result.Count} old passwords for account ID {accountId}.");
        }
        catch (Exception ex)
        {
            _logger.LogError($"Error retrieving old passwords for account ID {accountId}: {ex.Message}");
            return new ApiResponse<ICollection<ISA_PasswordsOld>>(null,
                $"Error retrieving old passwords for account ID {accountId}: {ex.Message}");
        }
    }
