...
public class BankAccountService : IBankAccountService
{
    ...
    public async Task<ApiResponse<ICollection<BankMasterData>>> GetActiveBankList()
    {
        ...
        catch (Exception ex)
        {
            ...
            return new ApiResponse<ICollection<BankMasterData>>(null,
                $"Error retrieving active bank list: {ex.Message}");
        }
    }
    ...
    public async Task<ApiResponse<BankMasterData>> UpdateBank(BankMasterData? item)
    {
        try
        {
            var existingBank = await _context.BankMasterData.Include(x => x.Country).Where(x => x.Id == item.Id)
                .FirstOrDefaultAsync();
    ...
    public async Task<ApiResponse<BankMasterData>> AddBank(BankMasterData? item)
    {
        ...
        try
        {
            ...
            return new ApiResponse<BankMasterData>(await _context.BankMasterData.FindAsync(item.Id),
                $"Bank with ID {item.Id} added successfully.");
        }
        catch (Exception ex)
        {
    ...
    public async Task<ApiResponse<bool>> DeleteBank(int id)
    {
        try
        {
            var existingBank = await _context.BankMasterData.Include(x => x.Country).Where(x => x.Id == id)
                .FirstOrDefaultAsync();
