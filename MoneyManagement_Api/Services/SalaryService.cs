...
public class SalaryService : ISalaryService
{
    ...
    public async Task<ApiResponse<Salary>> AddSalary(Salary item)
    {
        try
        {
            ...
            item.ExcengeRate = 1;
            if (currRate != null) item.ExcengeRate = currRate.Data.RateValue;
            ...
            return new ApiResponse<Salary>(await _context.Salary.FindAsync(item.Id),
                $"Salary added successfully with ID {item.Id}.");
        }
        catch (Exception ex)
        {
